...
function HomePopUp({ handleClick }: HomePopUp) {
  ...
  return (
    <>
      ...
      >
        <div className="absolute -top-24 -left-5 group py-5 w-16 hover:-mt-6">
          <SoundHover playType="hover" src="assets/original/sounds/star.mp3" vol={0.5}>
            <img
              className="group-hover:animate-bounce group-hover:cursor-pointer group-hover:drop-shadow-amber-300 group-hover:drop-shadow-xl size-16 block dark:hidden"
              src="assets/original/images/icon_star.webp"
            />
            <img
              className="group-hover:animate-bounce group-hover:cursor-pointer group-hover:drop-shadow-cyan-100 group-hover:drop-shadow-xl size-16 hidden dark:block"
        ...
        <div className="bg-window-bg w-[90vw] md:w-130 lg:w-180 h-[60vh] max-h-240 md:h-[60vh] md:max-h-120 border-2 border-window-border rounded-xl shadow-flat overflow-hidden flex flex-col">
          <div
          ...
          <div className='flex-1 min-h-0 overflow-y-auto'>
            <div className="p-6 flex flex-col gap-5">
                    ...
                    HomePageData?.map((item, key) => {
                      return (
                        <div className='h-10 group' key={key}>
                          <button
                            key={key}
                            className={`${activeTab === item?.title ? "shadow-flat bg-amber-50 dark:bg-linear-to-br dark:from-wave-from dark:to-wave-from/50 dark:bg-black dark:text-white" : ""} group-hover:mt-0.75 border-window-border border rounded-lg px-2 py-1 duration-150`}
              ...
              <div>
                <p className='text-amber-500 dark:text-cyan-100 font-semibold text-3xl'>
                  {
            ...
            <div className='flex items-center justify-center flex-wrap gap-5 mt-5 md:hidden'>
